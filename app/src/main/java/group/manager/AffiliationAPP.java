...
public class AffiliationAPP extends Activity {
	...
	SQLiteDatabase db;
	Cursor cursorT;
	String sqlSearch,Table2Name,Table4Name,Log,ClubName,logid,Str_user,StrClubName,StrCity,StrM_id,Strsrch,TableFamilyName,PName="";
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        Ivaddmember= (ImageView) findViewById(R.id.ivupdateMember);
	        ...
	        if(cursorT.getCount()==0){
	        	...
	        	rowItems = new ArrayList<RowEnvt>();
	        	 ...
	        	 if((StrCount==999999) || StrCount==111 ||(StrCount==888888)||(StrCount==22)||(StrCount==222)){
	        		 adapt = new CustomNews(context,R.layout.newslist, rowItems);
		             lvEvent.setAdapter(adapt); 
		             lvEvent.setSelection(post);
	        	 }else if(StrCount==888222||StrCount==888333){
	        		 adptfamily1 = new Customfamily1(context,R.layout.familylist1, rowItems);
		             lvEvent.setAdapter(adptfamily1);  
	        	 }
	        	 else{
	        		 adapter = new CustomAffil(context,R.layout.affiliationlist, rowItems);
